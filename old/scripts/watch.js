...
* ${pgjson.homepage}
* 
* (c) ${pgjson.watchjs.startyear || 2018}-${new Date().getFullYear()} ${
    pgjson.author.name
  }
...
*/`;
  ...
  if (result && result.code) {
    fs.writeFile(
      `dist/${noJSEnd(pgjson.name)}.min.js`,
      lib_infos + result.code,
      "utf8",
      (err) => {
        if (err) throw err;
  ...
  otherFiles &&
    otherFiles.forEach((str) => {
      ...
      if (match_arr && match_arr.length == 2) {
        ...
        // 新匹配内容
        let match_text_arr = basefile.match(
          new RegExp(
            `<o:start--${filename}-->([\\d\\D]+)//<o:end--${filename}-->`
          )
        );
